...
def create_sample_youtube_data():
    ...
    youtube_data = [
        {"title": "Fun Kids Animation", "description": "Colorful cartoon for children", "age_rating": 3},
        {"title": "Educational Math Tutorial", "description": "Learn basic arithmetic", "age_rating": 6},
        {"title": "Teen Dance Challenge", "description": "Popular dance moves tutorial", "age_rating": 13},
        {"title": "Horror Movie Trailer", "description": "Scary scenes and jump scares", "age_rating": 17},
        {"title": "Baby Lullaby Songs", "description": "Soothing music for babies", "age_rating": 0},
        {"title": "Action Movie Clips", "description": "Intense fight scenes", "age_rating": 15},
        {"title": "Science Experiment", "description": "Safe chemistry experiments", "age_rating": 8},
        {"title": "Gaming Strategy Guide", "description": "Advanced gaming tips", "age_rating": 12},
        {"title": "Cooking for Beginners", "description": "Simple recipe tutorial", "age_rating": 10},
        {"title": "Mature Documentary", "description": "Adult themes and content", "age_rating": 18},
        {"title": "Nursery Rhymes", "description": "Classic children's songs", "age_rating": 2},
        {"title": "Teen Fashion Tips", "description": "Style advice for teenagers", "age_rating": 14},
        {"title": "Adult Comedy Special", "description": "Mature humor and language", "age_rating": 18},
        {"title": "Elementary Science", "description": "Basic science concepts", "age_rating": 7},
        {"title": "High School Drama", "description": "Teen relationship stories", "age_rating": 13},
    ]
...
def register_custom_enrichers():
    ...
    def extract_youtube_features(data):
        ...
        for item in data:
            ...
            if any(word in title.lower() for word in ['kids', 'children', 'baby', 'nursery']):
                item['content_type'] = 'children'
            elif any(word in title.lower() for word in ['teen', 'teenager', 'high school']):
                item['content_type'] = 'teen'
            elif any(word in title.lower() for word in ['adult', 'mature', 'horror', 'comedy special']):
            ...
            intense_count = sum(1 for word in intense_words if word in (title + ' ' + description).lower())
            calm_count = sum(1 for word in calm_words if word in (title + ' ' + description).lower())
            
            item['intensity_score'] = intense_count - calm_count
            
            # Educational flag
            educational_words = ['tutorial', 'learn', 'educational', 'science', 'math', 'cooking']
            item['is_educational'] = any(word in (title + ' ' + description).lower() for word in educational_words)
...
def run_enterprise_optimization():
    ...
    results = optimize(
        ...
        # Advanced metrics
        metrics=["mae", "within_1", "within_2", "r2_score", "valid_rate", "prediction_confidence"],
