...
def _accuracy(predictions: List[Any], actuals: List[Any]) -> float:
    ...
    correct = sum(1 for p, a in zip(predictions, actuals) if str(p).lower() == str(a).lower())
...
def _binary_f1(predictions: List[str], actuals: List[str], positive_classes: List[str]) -> float:
